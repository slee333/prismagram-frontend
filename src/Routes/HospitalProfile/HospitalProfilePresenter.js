...
const Bio = styled.p`margin: 10px 0px;`;
...
export default ({ loading, data, logOut }) => {
	if (loading === true) {
		...
		const {
			seeHospital: { id, name, bio, files, location, staffs, admin, isYours, staffsCount, patientsCount }
		} = data;
		...
		return (
			<Wrapper>
             ...
             <Avatar size="lg" url={files[0].url} />
					...
					<HeaderColumn>
						{isYours ? (
							<Button onClick={console.log("I'm Youurs")} text="수정하기" />
						) : (
